...
function onKeyboardEvent(handler, event) {
  this.audioCtx.resume();

  const player = 1;
  switch (event.keyCode) {
    case 38: // UP
      handler(player, jsnes.Controller.BUTTON_UP);
      break;
    case 40: // Down
      handler(player, jsnes.Controller.BUTTON_DOWN);
      break;
    case 37: // Left
      handler(player, jsnes.Controller.BUTTON_LEFT);
      break;
    case 39: // Right
      handler(player, jsnes.Controller.BUTTON_RIGHT);
      break;
    case 65: // 'a' - qwerty, dvorak
    case 81: // 'q' - azerty
      handler(player, jsnes.Controller.BUTTON_A);
      break;
    case 83: // 's' - qwerty, azerty
    case 79: // 'o' - dvorak
      handler(player, jsnes.Controller.BUTTON_B);
      break;
    case 9: // Tab
      handler(player, jsnes.Controller.BUTTON_SELECT);
      break;
    case 13: // Return
      handler(player, jsnes.Controller.BUTTON_START);
      break;
    default:
      break;
  }
...
document.addEventListener("keydown", event => {
  onKeyboardEvent(nes.buttonDown, event);
});

document.addEventListener("keyup", event => {
  onKeyboardEvent(nes.buttonUp, event);
});
