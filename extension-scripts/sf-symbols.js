...
let selecteIndex = 0;
let searchKey = "";
let visibleItems = symbols[selecteIndex].items;
  ...
  views: [
    {
      ...
      events: {
        changed: sender => {
          searchKey = sender.text.toLowerCase();
          reloadData();
        }
      }
    },
    {
      ...
      props: {
        id: "menu",
        segmentWidthStyle: 1,
        items: symbols.map($0 => `  ${$0.title} (${$0.items.length})  `)
      },
      layout: (make, view) => {
    ...
    },
    {
      ...
      events: {
        didSelect: async(sender, indexPath) => {
          const symbol = visibleItems[indexPath.item];
          $ui.alert({
            title: symbol,
            actions: [
              {
...
function reloadData() {
  const category = symbols[selecteIndex].items;
  visibleItems = searchKey.length == 0 ? category : category.filter(symbol => {
    return symbol.includes(searchKey);
  });

  const view = $("collection");
  view.data = map(visibleItems);
  view.contentOffset = $point(0, 0);
}
...
function map(data) {
  return data.map(symbol => {
    return {
      ...
      icon: {
        image: $objc("UIImage").$systemImageNamed(symbol).jsValue()
      }
    }
  })
}