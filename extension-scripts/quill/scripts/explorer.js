        ...
        actions: [
          {
            ...
            handler: () => {
              ...
              const html = items.source;
              saveWebPage(name, html, folder);
            }
          },
          {
            title: $l10n("CANCEL"),
...
function reloadFolder(folder, throttle=false) {
  ...
  if (list) {
    list.data = files.map(file => {
      let image = file.isDirectory ? "folder" : "text";
      return {
        "icon": {
          "src": `assets/${image}.png`
        },
        "label": {
          "text": util.removePathExtension(file.displayName || file.name)
        }
      }
    });
  }
...
function saveWebPage(name, html, folder) {
  const path = util.toQuillFile(util.removePathExtension(name));
  const converter = require("./editor/converter");
  converter.convert(html, data => {
    saveFile(data, path, folder);
  });
}