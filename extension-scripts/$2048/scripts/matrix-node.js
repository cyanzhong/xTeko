...
function setup() {
  ...
  self.$addChild(sweepAudioNode);
  self.$setSweepAudioNode(sweepAudioNode);

  var sumAudioNode = builder.audioNode("sfx_sum.mp3");
  self.$addChild(sumAudioNode);
  self.$setSumAudioNode(sumAudioNode);
...
function createItem() {
  ...
  var node = $objc("MatrixItem").$alloc().$initWithNumber_size(number, size);
  node.$setPosition(nodePosition(r, c));
  node.$showIn(self);
...
function swipeTo(direction) {
  ...
  if (combineTo(direction)) {
    helper.playAudio(self.$sumAudioNode());
  }
  ...
  // Play audio
  helper.playAudio(self.$sweepAudioNode());

  if (gameOver()) {
    self.$eventHandler().$didFinish(false);
  }
}
  ...
  for (var i=vx[0][idx]; i<4&&i>=0; i+=vx[1][idx]) {
    ...
    for (var j=vy[0][idx]; j<4&&j>=0; j+=vy[1][idx]) {
      ...
      if (matched) {
        ...
        self.$eventHandler().$scoreChanged(score);

        // You win
        if (score >= 2048) {
          self.$eventHandler().$didFinish(true);
        }
      }
    }
  }
...
function gameOver() {
  ...
  return self.$children().$count() >= maxChildrenNumber;
}
